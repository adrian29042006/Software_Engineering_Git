...
# --- SPRINT 3: Erweiterung TimerManager ---
class TimerManager:
    ...
    def delayed_display_update(self):
        # Anzeigeverzögerung max. 500 ms
        root.after(500, lambda: self.led_display.update_timer(self.seconds))


# --- SPRINT 3: Erweiterung TimerController ---
class TimerController:
    ...
    def increase_time(self):
        if self.current_minutes < 20:
            self.current_minutes += 1
            self.timer_manager.set_time(self.current_minutes)

    def decrease_time(self):
        if self.current_minutes > 1:
            self.current_minutes -= 1
            self.timer_manager.set_time(self.current_minutes)
...
# --- SPRINT 3: GUI-Erweiterung Kochzeit ---
timer_manager = TimerManager(led_display)
timer_controller = TimerController(timer_manager)

timer_plus_btn = tk.Button(root, text="⏱ +", command=timer_controller.increase_time)
timer_plus_btn.grid(row=6, column=0)

timer_minus_btn = tk.Button(root, text="⏱ -", command=timer_controller.decrease_time)
timer_minus_btn.grid(row=6, column=2)
